<?php
class Purchasertn extends CI_Controller
{
  ...
  public function __construct()
  {
    ...
    $this->load->model('Mdl_purchasertn');
  }
  ...
  public function index()
  {
    if(isset($_SESSION['userDetails']))
    {
      ...
      $this->load->view('universe/purchasertn_reg');
...
public function verify_purchasertname()
  {
    if(isset($_SESSION['userDetails'])&&$this->uri->segment(3)==true)
    {
      $purchasertname=stripslashes($this->uri->segment(3));
      if (!preg_match('/[^A-Za-z]/', $purchasertname))
      {
        $this->session->set_userdata('purchasertname_exist',1);
        echo true;
      }
      else
      {
  ...
  function get_agrphname($stag)
  {
    if(isset($_SESSION['userDetails'])&&$this->uri->segment(3)==true)
    {
      ...
      if($res=$this->Mdl_purchasertn->get_agrphname($stag))
      {
        $data=array();
        // print_r($res);
        foreach($res as $row)
  ...
  public function purchasertn_reg()
  {
    if(isset($_SESSION['userDetails']))
    {
      if(isset($_SESSION['purchasertname_exist'])&&isset($_SESSION['ss_registration']))
      {
        $grphname=$this->input->post('grphname');
        $ss_registered=$this->input->post('registered_ss');
        $memid=$_SESSION['userDetails']['regid'];
        $purchasertname=$this->input->post('purchasertname');
        ...
        $data=array(
          ...
          'member_uniqid'=>$memid,
          'purchasertname'=>$purchasertname,
          'purchasertnuid'=>$purchasertnuid,
          
        );

        if($this->Mdl_purchasertn->purchasertn_reg($data))
        {
          echo "<center><h2>Successfully registered <b>".$purchasertname."</b>!</h2><br><a href='javascript:history.go(-1)'>Go back</a></center>";
          $this->clear_all();
        }
        else
        {
  ...
  public function verify_agrphname()
  {
    if(isset($_SESSION['userDetails'])&&$this->uri->segment(3)==true)
    {
      ...
      if($this->Mdl_purchasertn->verify_agrphname($agrphname))
      {
