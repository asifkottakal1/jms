...
class Sales extends CI_Controller
{
  ...
  public function index()
  {
    if(isset($_SESSION['userDetails']))
    {
      ...
      $this->load->view('universe/sales_reg');
...
public function verify_salesname()
  {
    if(isset($_SESSION['userDetails'])&&$this->uri->segment(3)==true)
    {
      $salesname=stripslashes($this->uri->segment(3));
      if (!preg_match('/[^A-Za-z]/', $salesname))
      {
        $this->session->set_userdata('salesname_exist',1);
        echo true;
      }
      else
      {
  ...
  function get_agrphname($stag)
  {
    if(isset($_SESSION['userDetails'])&&$this->uri->segment(3)==true)
    {
      ...
      if($res=$this->Mdl_sales->get_agrphname($stag))
      {
        $data=array();
        // print_r($res);
        foreach($res as $row)
  ...
  public function sales_reg()
  {
    if(isset($_SESSION['userDetails']))
    {
      if(isset($_SESSION['salesname_exist'])&&isset($_SESSION['ss_registration']))
      {
        ...
        if($this->Mdl_sales->sales_reg($data))
        {
          echo "<center><h2>Successfully registered <b>".$salesname."</b>!</h2><br><a href='javascript:history.go(-1)'>Go back</a></center>";
          $this->clear_all();
        }
        else
        {
  ...
  public function verify_agrphname()
  {
    if(isset($_SESSION['userDetails'])&&$this->uri->segment(3)==true)
    {
      ...
      if($this->Mdl_sales->verify_agrphname($agrphname))
      {
