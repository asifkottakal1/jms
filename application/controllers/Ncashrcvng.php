<?php
class Ncashrcvng extends CI_Controller
{
  ...
  public function __construct()
  {
    ...
    $this->load->model('Mdl_ncashrcvng');
  }
  ...
  public function index()
  {
    if(isset($_SESSION['userDetails']))
    {
      ...
      $this->load->view('universe/ncashrcvng_reg');
  ...
  function get_areauid($stag)
  {
    if(isset($_SESSION['userDetails'])&&$this->uri->segment(3)==true)
    {
      ...
      if($res=$this->Mdl_ncashrcvng->get_areauid($stag))
      {
        $data=array();
        // print_r($res);
        foreach($res as $row)
  ...
  public function ncashrcvng_reg()
  {
    if(isset($_SESSION['userDetails']))
    {
      if(isset($_SESSION['member_exist'])&&isset($_SESSION['ss_registration'])&&isset($_SESSION['areauid_exist']))
      {
        ...
        $rcvngacc=$this->input->post('rcvngacc');
        $gmode=$this->input->post('gmode');
        $rcvngitem=$this->input->post('rcvngitem');
        $quantity=$this->input->post('quantity');
        $itemvaluation=$this->input->post('itemvaluation');
        ...
        if($this->Mdl_ncashrcvng->ncashrcvng_reg($data))
        {
  ...
  public function verify_areauid()
  {
    if(isset($_SESSION['userDetails'])&&$this->uri->segment(3)==true)
    {
      ...
      if($this->Mdl_ncashrcvng->verify_areauid($areauid))
      {
  ...
  function get_sabha($sabha)
  {
    if(isset($_SESSION['userDetails'])&&$this->uri->segment(3)==true)
    {
      ...
      if($res=$this->Mdl_ncashrcvng->get_sabha($sabha))
      {
  ...
  public function verify_user()
  {
    ...
    { 
      ...
      if($res=$this->Mdl_ncashrcvng->verify_user($data))
      {
