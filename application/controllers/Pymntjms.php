<?php
class Pymntjms extends CI_Controller
{
  ...
  public function index()
  {
    if(isset($_SESSION['userDetails']))
    {
      ...
      $this->load->view('universe/pymntjms_reg');
  ...
  function get_areauid($stag)
  {
    if(isset($_SESSION['userDetails'])&&$this->uri->segment(3)==true)
    {
      ...
      if($res=$this->Mdl_pymntjms->get_areauid($stag))
      {
        $data=array();
        // print_r($res);
        foreach($res as $row)
  ...
  public function pymntjms_reg()
  {
    if(isset($_SESSION['userDetails']))
    {
      if(isset($_SESSION['member_exist'])&&isset($_SESSION['ss_registration'])&&isset($_SESSION['areauid_exist']))
      {
        ...
        if($this->Mdl_pymntjms->pymntjms_reg($data))
        {
  ...
  public function verify_areauid()
  {
    if(isset($_SESSION['userDetails'])&&$this->uri->segment(3)==true)
    {
      ...
      if($this->Mdl_pymntjms->verify_areauid($areauid))
      {
  ...
  function get_sabha($sabha)
  {
    if(isset($_SESSION['userDetails'])&&$this->uri->segment(3)==true)
    {
      ...
      if($res=$this->Mdl_pymntjms->get_sabha($sabha))
      {
  ...
  public function verify_user()
  {
    ...
    { 
      ...
      if($res=$this->Mdl_pymntjms->verify_user($data))
      {
