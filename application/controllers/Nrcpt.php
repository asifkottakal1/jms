<?php
class Nrcpt extends CI_Controller
{
  ...
  public function index()
  {
    if(isset($_SESSION['userDetails']))
    {
      ...
      $this->load->view('universe/nrcpt_reg');
...
public function verify_nrcptname()
  {
    if(isset($_SESSION['userDetails'])&&$this->uri->segment(3)==true)
    {
      $nrcptname=stripslashes($this->uri->segment(3));
      if (!preg_match('/[^A-Za-z]/', $nrcptname))
      {
        $this->session->set_userdata('nrcptname_exist',1);
        echo true;
      }
      else
      {
  ...
  function get_agrphname($stag)
  {
    if(isset($_SESSION['userDetails'])&&$this->uri->segment(3)==true)
    {
      ...
      if($res=$this->Mdl_nrcpt->get_agrphname($stag))
      {
        $data=array();
        // print_r($res);
        foreach($res as $row)
  ...
  public function nrcpt_reg()
  {
    if(isset($_SESSION['userDetails']))
    {
      if(isset($_SESSION['nrcptname_exist'])&&isset($_SESSION['ss_registration']))
      {
        $grphname=$this->input->post('grphname');
        $ss_registered=$this->input->post('registered_ss');
        $memid=$_SESSION['userDetails']['regid'];
        $nrcptname=$this->input->post('nrcptname');
 
        $nrcptuid=$this->input->post('nrcptuid');
        ...
        if($this->Mdl_nrcpt->nrcpt_reg($data))
        {
          echo "<center><h2>Successfully registered <b>".$nrcptname."</b>!</h2><br><a href='javascript:history.go(-1)'>Go back</a></center>";
          $this->clear_all();
        }
        else
        {
...
public function verify_agrphname()
  {
    if(isset($_SESSION['userDetails'])&&$this->uri->segment(3)==true)
    {
      ...
      if($this->Mdl_nrcpt->verify_agrphname($agrphname))
      {
