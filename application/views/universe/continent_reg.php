 <div class="content-wrapper">
    <div class="container-fluid">
      ...
      <form method="post" action="<?php echo base_url()?>continent/continent_reg">
      ...
      <div class="row parent_space" style="text-align: center;">
        <div class="col-sm-3">
          <label class="label_left">Planet</label>
        </div>
        <div class="col-sm-6">
          <input type="text" name="planame" id="planame" class="form-control" required placeholder="Type Ucode & Confirm or Search & Confirm">
        </div>
        <div class="col-sm-3">
      ...
      <div class="row row_space">
        <div class="col-sm-4">
          <label class="label_left">Name of Continent</label>
        </div>
        <div class="col-sm-4">
          <input type="text" name="coname" id="coname" class="form-control" required placeholder="Type name">
        </div>
      </div>
      ...
      <div class="row row_space">
        <div class="col-sm-4">
          <label class="label_left">Capital of Continent</label>
        </div>
        <div class="col-sm-4">
          <input type="text" name="concapital" id="concapital" class="form-control" required placeholder="Type Ucode & Confirm or Search & Confirm">
        </div>
        <div class="col-sm-3">
...
<script type="text/javascript">
  ...
  window.onload=function()
    {
        $("#planame").autocomplete({
            source:['']
        });
        $('#planame').keyup(function(){
            ssid=$('#planame').val();
            $.ajax ({
                method: 'post',
                url:'<?=base_url('planet/get_uniqid/') ?>'+ssid,
  ...
  {
    if($('#concapital').val()!=''&&$('#concapital').val()!=' ')
    {
      $('#concapital').attr('readonly',true);
      $('#concapital').attr('onmousedown','return false');
  ...
  {
    universe=$('#planame').val();
    $.ajax ({
      method: 'post',
      url: "<?php echo base_url() ?>planet/verify_planet/"+universe,
      success: function(data)
      {
        if(data=='ok')
        {
            $('#planame').attr('readonly',true);
            $('#planame').attr('onmousedown','return false');
