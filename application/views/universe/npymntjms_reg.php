    ...
    <div class="container-fluid">
      ...
      <ol class="breadcrumb">
        <li class="breadcrumb-item">
          <a href="#"><b>Non Cash Expences/Payment For Janamahasabha or Councils Register</b></a>
        </li>
        <li class="breadcrumb-item active">My Dashboard</li>
      </ol>

      <form method="post" action="<?php echo base_url()?>npymntjms/npymntjms_reg">
              <div class="col-sm-4">
...
<div class="col-sm-4">
      ...
      <div class="row row_space">
        <div class="col-sm-3">
          <label class="label_left">Non Cash item paid to</label>
        </div>
        <div class="col-sm-6">
          <input type="text" name="memid" id="memid" class="form-control" required placeholder="Type MUI (MembershipUnique ID)">
        </div>
      ...
      <!--  <div class="row row_space">
        ...
        <div class="col-sm-5">
         ...
         <input name='drcr' id='credit' type='radio' value="credit" onclick="$('#amountLabel').text('Credit Amount')">cr(Cash Out)
             ...
             <div class="row row_space">
        <div class="col-sm-3">
          <label class="label_left">Non Cash item Expence Account</label>
        </div>
               <div class="col-sm-6">
          <select name="nexpacc" id="nexpacc" required class="form-control">
      ...
      <div class="row row_space">
        <div class="col-sm-3">
          <label class="label_left">Non Cash Paid mode</label>
        </div>
               <div class="col-sm-6">
          <select name="npaidmode" id="npaidmode" required class="form-control">
             <option value="">---Select---</option>
             <option value='cash'>Ready Handover</option>
   ...
   <div class="row row_space">
        <div class="col-sm-3">
          <label class="label_left">Paid Item</label>
        </div>
               <div class="col-sm-6">
          <select name="pitem" id="pitem" required class="form-control">
             <option value="">---Select---</option>
             <option value='cash'>Item</option>
        ...
        <div class="col-sm-1">
          <label class="label_right">Quantity</label>
        </div>
       <div class="col-sm-2">
      ...
      <div class="row row_space ">
        <div class="col-sm-3">
          <label class="label_left">Expence Item Aprx Valuation</label>
        </div>
        <div class="col-sm-3">
          <input type="text" class="form-control" name="valuation" id="valuation">
        </div>
      </div>
      <div class="row row_space ">
        <div class="col-sm-3">
...
<script type="text/javascript">
  ...
  window.onload=function()
    {
        ...
        $('#areauid').keyup(function(){
            ...
            $.ajax ({
                method: 'post',
                url:'<?=base_url('npymntjms/get_areauid/') ?>'+ssid,
                success: function(data)
                {
                    if(data)
                    {
    ...
    function verify_who(z)
    {
      if(z=='member')
      {
        ...
        $.ajax ({
          method: 'post',
          url: "npymntjms/verify_user/"+memid,
          success: function(data)
          {
              if(data)
              {
  ...
  areauid=$('#areauid').val();
    $.ajax ({
      method: 'post',
      url: "<?php echo base_url() ?>npymntjms/verify_areauid/"+areauid,
      success: function(data)
      {
        // console.log(data)
        if(data==1)
        {
            ...
            alert('Verified');
            $.ajax({
              url:"<?php echo base_url() ?>npymntjms/get_sabha/"+areauid,
              success:function(data){
                data=JSON.parse(data);
                console.log(data)
                html='';
