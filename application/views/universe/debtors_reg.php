    ...
    <div class="container-fluid">
      ...
      <ol class="breadcrumb">
        <li class="breadcrumb-item">
          <a href="#"><b>Debtors Account Head Creation</b></a>
        </li>
        <li class="breadcrumb-item active">My Dashboard</li>
      </ol>

      <form method="post" action="<?php echo base_url()?>debtors/debtors_reg">
              <div class="col-sm-4">
...
<div class="col-sm-4">
      ...
      <div class="row parent_space" style="text-align: center;">
        <div class="col-sm-3">
          <label class="label_left">Debitors Account Head Name</label>
        </div>
        <div class="col-sm-5">
          <input type="text" name="debtname" id="debtname" class="form-control" required placeholder="Type">
        </div>
        
       <div class="col-sm-3">
           <button type="button" class="verify_btn verify_debtname" onclick="verify_who('debtname')">Verify & Confirm</button>
        </div>
      ...
      <div class="row parent_space" style="text-align: center;">
        <div class="col-sm-3">
          <label class="label_left">Debtors Account Head U Code</label>
        </div>
        <div class="col-sm-5">
          <input type="text" name="debtuid" id="debtuid" class="form-control" required placeholder="Type">
        </div>
...
<script type="text/javascript">
  ...
  window.onload=function()
    {
        ...
        $('#grphname').keyup(function(){
            ...
            $.ajax ({
                method: 'post',
                url:'<?=base_url('debtors/get_agrphname/') ?>'+ssid,
                success: function(data)
                {
                    if(data)
                    {
    ...
    function verify_who(z)
    {
      ...
      {
         debtname=$('#debtname').val();
         $.ajax ({
            method: 'post',
            url: "Debtors/verify_debtname/"+debtname,
            success: function(data)
            {
              if(data)
              {
                  alert('Verified!');
                  $('#debtname').attr('readonly',true);
                  $('.verify_debtname').css('display','none');
                  $('#debtname').attr('onmousedown','return false');
              }
              else
              {
                  alert("Invalid! ");   
     ...
     function verify_agrphname()
  {
   grphname=$('#grphname').val();
    $.ajax ({
      method: 'post',
      url: "<?php echo base_url() ?>debtors/verify_agrphname/"+grphname,
      success: function(data)
      {
        // console.log(data)
        if(data==1)
