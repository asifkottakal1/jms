    ...
    <div class="container-fluid">
      ...
      <ol class="breadcrumb">
        <li class="breadcrumb-item">
          <a href="#"><b>Sales Return Account Head Creation</b></a>
        </li>
        <li class="breadcrumb-item active">My Dashboard</li>
      </ol>

      <form method="post" action="<?php echo base_url()?>salesrtn/salesrtn_reg">
              <div class="col-sm-4">
...
<div class="col-sm-4">
      ...
      <div class="row parent_space" style="text-align: center;">
        <div class="col-sm-3">
          <label class="label_left">Sales Return Account Head Name</label>
        </div>
        <div class="col-sm-5">
          <input type="text" name="salesrtname" id="salesname" class="form-control" required placeholder="Type">
        </div>
        
       <div class="col-sm-3">
           <button type="button" class="verify_btn verify_salesrtname" onclick="verify_who('salesrtname')">Verify & Confirm</button>
        </div>
...
<script type="text/javascript">
  ...
  window.onload=function()
    {
        ...
        $('#grphname').keyup(function(){
            ...
            $.ajax ({
                method: 'post',
                url:'<?=base_url('salesrtn/get_agrphname/') ?>'+ssid,
                success: function(data)
                {
                    if(data)
                    {
    ...
    function verify_who(z)
    {
      if(z=='member')
      {
       ...
       else if(z=='salesrtname')
      {
         salesrtname=$('#salesrtname').val();
         $.ajax ({
            method: 'post',
            url: "salesrtn/verify_salesrtname/"+salesrtname,
            success: function(data)
            {
              if(data)
              {
                  alert('Verified!');
                  $('#salesrtname').attr('readonly',true);
                  $('.verify_salesrtname').css('display','none');
                  $('#salesrtname').attr('onmousedown','return false');
              }
              else
              {
                  alert("Invalid! ");   
     ...
     function verify_agrphname()
  {
   grphname=$('#grphname').val();
    $.ajax ({
      method: 'post',
      url: "<?php echo base_url() ?>salesrtn/verify_agrphname/"+grphname,
      success: function(data)
      {
        // console.log(data)
        if(data==1)
