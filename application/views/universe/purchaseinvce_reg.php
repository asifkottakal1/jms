    ...
    <div class="container-fluid">
      ...
      <ol class="breadcrumb">
        <li class="breadcrumb-item">
          <a href="#"><b>Purchase Invoice Register For  JMS </b></a>
        </li>
        <li class="breadcrumb-item active">My Dashboard</li>
      </ol>

      <form method="post" action="<?php echo base_url()?>purchaseinvce/purchaseinvce_reg">
              <div class="col-sm-4">
...
<div class="col-sm-4">
      ...
      <div class="row row_space" >
        <div class="col-sm-3">
          <label class="label_left">Purchase From</label>
        </div>
        <div class="col-sm-6">
          <input type="text" name="purchaseuid" id="purchaseuid" class="form-control" required placeholder="Type">
        </div>
        
       <div class="col-sm-3">
           <button type="button" class="verify_btn verify_purchaseuid" onclick="verify_who('purchaseuid')">Verify & Confirm</button>
        </div>
...
<div class="row row_space" >
        <div class="col-sm-3">
          <label class="label_left">Purchase Account Head</label>
        </div>
         <div class="col-sm-6">
          <select name="purchase" id="purchase" required class="form-control">
          </select>
        </div>
...
<div class="more" style="visibility: none;" >
  ...
  <div class="row row_space">
    ...
    <div class="col-sm-4">Purchase Item</div>
 ...
 <div class="row row_space" >
   <div class="col-sm-1">
        ...
        <div class="col-sm-4">
          <input type="text" name="purchaseitm[]"  class="form-control purchaseitm" required placeholder="Type Item Code">
        </div>
...
<input type="text" name="totalAmount" id="totalAmount" class="form-control"  placeholder="">
 ...
 <div class="row row_space" >
    ...
    <div class="col-sm-1">
        ...
        <div class="col-sm-4">
          <input type="text" name="purchaseitm[]"  class="form-control purchaseitm" placeholder="Type Item Code">
        </div>
     <div class="col-sm-1 quantity-container">
          <input type="text" name="quantity[]"  class="form-control quantity" placeholder="" value='0'>
        </div>
  ...
  window.onload=function()
    {
        ...
        $('#areauid').keyup(function(){
            ...
            $.ajax ({
                method: 'post',
                url:'<?=base_url('purchaseinvce/get_areauid/') ?>'+ssid,
                success: function(data)
                {
                    if(data)
                    {
...
$("#purchaseuid").autocomplete({
            source:['']
        });
        $('#purchaseuid').keyup(function(){
            ssid=$('#purchaseuid').val();
            $.ajax ({
                method: 'post',
                url:'<?=base_url('purchaseinvce/get_purchaseuid/') ?>'+ssid,
                success: function(data)
                {
                    if(data)
                    {
    ...
    function verify_who(z)
    {
      if(z=='member')
      {
       ...
       else if(z=='purchaseuid')
      {
         purchaseuid=$('#purchaseuid').val();
         $.ajax ({
            method: 'post',
            url: "purchaseinvce/verify_purchaseuid/"+purchaseuid,
            success: function(data)
            {
              if(data)
              {
