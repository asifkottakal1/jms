    ...
    <div class="container-fluid">
      ...
      <ol class="breadcrumb">
        <li class="breadcrumb-item">
          <a href="#"><b>Purchase Return Invoice Register For  JMS </b></a>
        </li>
        <li class="breadcrumb-item active">My Dashboard</li>
      </ol>

      <form method="post" action="<?php echo base_url()?>purchasertnin/purchasertnin_reg">
              <div class="col-sm-4">
...
<div class="col-sm-4">
      ...
      <div class="row row_space" >
        <div class="col-sm-3">
          <label class="label_left">Purchase Return To</label>
        </div>
        <div class="col-sm-6">
          <input type="text" name="purchasertnuid" id="purchasertnuid" class="form-control" required placeholder="Type">
        </div>
        
       <div class="col-sm-3">
           <button type="button" class="verify_btn verify_purchasertnuid" onclick="verify_who('purchasertnuid')">Verify & Confirm</button>
        </div>
...
<div class="row row_space" >
        <div class="col-sm-3">
          <label class="label_left">Purchase Return Account Head</label>
        </div>
         <div class="col-sm-6">
          <select name="purchasertn" id="purchasertn" required class="form-control">
          </select>
        </div>
     ...
     <div class="col-sm-4">
          <label class="label_left"><h4>Purchase Details</h4></label>
        </div>
      <div>
       <div>       
...
<div class="more" style="visibility: none;" >
  ...
  <div class="row row_space">
    ...
    <div class="col-sm-4">Purchase Return Item</div>
...
Total Quantity</div>
<div class="col-sm-1">
<input type="text" name="totalQuantity" id="totalQuantity" class="form-control" required placeholder="">
</div>
...
<input type="text" name="totalAmount" id="totalAmount" class="form-control"  placeholder="">
 ...
 <div class="row row_space" >
        ...
        <div class="col-sm-9">
          <input type="text" name="purchase" id="purchase" class="form-control" placeholder="Type">
        </div>
      ...
      <div style="display: none">
        <div class="row row_space purchase-details">
    <div class="col-sm-1">
          <input type="text" name="sno[]"  class="form-control sno" placeholder="">
        </div>
        <div class="col-sm-4">
  ...
  window.onload=function()
    {
        ...
        $('#areauid').keyup(function(){
            ...
            $.ajax ({
                method: 'post',
                url:'<?=base_url('purchasertnin/get_areauid/') ?>'+ssid,
                success: function(data)
                {
                    if(data)
                    {
...
$("#purchasertnuid").autocomplete({
            source:['']
        });
        $('#purchasertnuid').keyup(function(){
            ssid=$('#purchasertnuid').val();
            $.ajax ({
                method: 'post',
                url:'<?=base_url('purchasertnin/get_purchasertnuid/') ?>'+ssid,
                success: function(data)
                {
                    if(data)
                    {
          ...
          $('#add').click(function(){
            $('.purchase-details:last').clone().appendTo('#purchase-details-div');
            $("#purchase-details-div input[type=text]").prop('required',true);

          })

          $('#purchase-details-div').on('click','.remove-btn',function(){
            $(this).closest('.purchase-details').remove();
          })

          $('#purchase-details-div').on('change','.quantity',function(){
            sum=0;
          ...
          $('#purchase-details-div').on('change','.price',function(){
            price=$(this).val();
          ...
          $("#purchase-details-div").on('change','.amount',function(){
            sum=0;
    ...
    function verify_who(z)
    {
      if(z=='member')
      {
       ...
       else if(z=='purchasertnuid')
      {
         purchasertnuid=$('#purchasertnuid').val();
         $.ajax ({
            method: 'post',
            url: "purchasertnin/verify_purchasertnuid/"+purchasertnuid,
            success: function(data)
            {
              if(data)
              {
                  
                  $('#purchasertnuid').attr('readonly',true);
                  $('.verify_purchasertnuid').css('display','none');
                  $('#purchasertnuid').attr('onmousedown','return false');
                  alert('Verified!');
                  $.ajax({
              url:"<?php echo base_url() ?>purchasertnin/get_purchasertn/"+purchasertnuid,
              success:function(data){
                data=JSON.parse(data);
                console.log(data)
                html='';
                ...
                {
                  html+="<option value='"+data[i]['prtid']+"'>"+data[i]['purchasertn']+"</option>"
                }
                $('#purchasertn').html(html)
              }
            })
        }
        else if(data=='no')
  ...
  areauid=$('#areauid').val();
    $.ajax ({
      method: 'post',
      url: "<?php echo base_url() ?>purchasertnin/verify_areauid/"+areauid,
      success: function(data)
      {
        // console.log(data)
        if(data)
        {
            ...
            alert('Verified');
            $.ajax({
              url:"<?php echo base_url() ?>purchasertnin/get_sabha/"+areauid,
              success:function(data){
                data=JSON.parse(data);
                console.log(data)
                html='';
