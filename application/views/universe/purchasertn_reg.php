    ...
    <div class="container-fluid">
      ...
      <ol class="breadcrumb">
        <li class="breadcrumb-item">
          <a href="#"><b>Purchase return Account Head Creation</b></a>
        </li>
        <li class="breadcrumb-item active">My Dashboard</li>
      </ol>

      <form method="post" action="<?php echo base_url()?>purchasertn/purchasertn_reg">
              <div class="col-sm-4">
...
<div class="col-sm-4">
      ...
      <div class="row parent_space" style="text-align: center;">
        <div class="col-sm-3">
          <label class="label_left">Purchase Return Account Head Name</label>
        </div>
        <div class="col-sm-5">
          <input type="text" name="purchasertname" id="purchasertname" class="form-control" required placeholder="Type">
        </div>
        
       <div class="col-sm-3">
           <button type="button" class="verify_btn verify_purchasertname" onclick="verify_who('purchasertname')">Verify & Confirm</button>
        </div>
      ...
      <div class="row parent_space" style="text-align: center;">
        <div class="col-sm-3">
          <label class="label_left">Purchase Return Account Head U Code</label>
        </div>
        <div class="col-sm-5">
...
<script type="text/javascript">
  ...
  window.onload=function()
    {
        ...
        $('#grphname').keyup(function(){
            ...
            $.ajax ({
                method: 'post',
                url:'<?=base_url('purchasertn/get_agrphname/') ?>'+ssid,
                success: function(data)
                {
                    if(data)
                    {
    ...
    function verify_who(z)
    {
      if(z=='member')
      {
       ...
       else if(z=='purchasertname')
      {
         purchasertname=$('#purchasertname').val();
         $.ajax ({
            method: 'post',
            url: "purchasertn/verify_purchasertname/"+purchasertname,
            success: function(data)
            {
              if(data)
              {
                  alert('Verified!');
                  $('#purchasertname').attr('readonly',true);
                  $('.verify_purchasertname').css('display','none');
                  $('#purchasertname').attr('onmousedown','return false');
              }
              else
              {
                  alert("Invalid! ");   
     ...
     function verify_agrphname()
  {
   grphname=$('#grphname').val();
    $.ajax ({
      method: 'post',
      url: "<?php echo base_url() ?>purchasertn/verify_agrphname/"+grphname,
      success: function(data)
      {
        // console.log(data)
        if(data==1)
